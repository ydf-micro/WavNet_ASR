...
class WaveNet():
    def __init__(self, n_out,  batch_size=1, n_mfcc=20, is_training=True):
        ...
        self.input_data = tf.placeholder(dtype=tf.float32,
                                         shape=[batch_size, None, n_mfcc])
        self.seq_len = tf.reduce_sum(tf.cast(
            tf.not_equal(
                tf.reduce_sum(self.input_data,
                              reduction_indices=2),
                0.),
            tf.int32),
            reduction_indices=1)
    ...
    def conv1d_layer(self, input_tensor, size=1,
                     dim=128, bias=False, activation='tanh'):
        with tf.variable_scope('conv1d' + str(self.conv1d_index)):
            shape = input_tensor.get_shape().as_list()
            kernel = tf.get_variable('kernel', (size, shape[-1], dim),
                                     dtype=tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer())
            if bias:
                b = tf.get_variable('b', [dim], dtype=tf.float32,
                                    initializer=tf.constant_initializer(0))
            out = tf.nn.conv1d(input_tensor, kernel, stride=1, padding='SAME') \
                  + (b if bias else 0)
            if not bias:
    ...
    def aconv1d_layer(self, input_tensor, size=7,
                      rate=2, bias=False, activation='tanh'):
        with tf.variable_scope('aconv1d_' + str(self.aconv1d_index)):
            shape = input_tensor.get_shape().as_list()
            kernel = tf.get_variable('kernel', (1, size, shape[-1], shape[-1]),
                                     dtype=tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer())

            out = tf.nn.atrous_conv2d(tf.expand_dims(input_tensor, axis=1),
                                      kernel, rate=rate, padding='SAME')
